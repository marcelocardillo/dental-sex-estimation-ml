# ==============================================================================
# SCRIPT INTEGRADO: AJUSTE DE MODELOS PREDICTIVOS (VERSIÓN FINAL)
# Descripción: Entrenamiento y evaluación de múltiples modelos de clasificación
#              para predecir Sexo a partir de medidas dentales (CS y CI)
# ==============================================================================

# --- 1. CARGA DE PAQUETES -----------------------------------------------------
# Se cargan todas las librerías necesarias para el ecosistema tidymodels,
...
library(ggbeeswarm)
library(boot)
library(viridis)  # Para escalas de color en gráficos
...
# --- 3. FILTRADO PARA CANINOS SUPERIORES (CS) ---------------------------------
...
# --- 4. FUNCIÓN DE DIVISIÓN DE DATOS ------------------------------------------
# Función para dividir datos manteniendo la integridad por individuo y sexo
# Esto evita pseudorreplicación (mismo individuo en train y test)
...
length(intersect(unique(cs_train$Ind), unique(cs_test$Ind)))  # Debe ser 0

# --- 5. LIMPIEZA DE DATOS (CS) ------------------------------------------------
...
cs_train %>%
  filter(is.na(MD) & is.na(BL) & is.na(MDCu) & is.na(BLCu))  # Debe ser 0 filas

# --- 6. PREPROCESAMIENTO (RECIPE) ---------------------------------------------
# Crear receta para preprocesamiento de datos
# Incluye imputación KNN y normalización de predictores
...
# --- 15. MODELO 8: XGBOOST (MEJOR MODELO CS) ----------------------------------
...
metricas_clase <- metric_set(accuracy, sens, spec, bal_accuracy)
...
# --- 18. BOOTSTRAP PARA INTERVALOS DE CONFIANZA (CS) --------------------------
...
# CORRECCIÓN: boot.ci devuelve c(conf_level, lower, upper) en $percent
...
# --- 20. IMPORTANCIA DE VARIABLES (CS) ----------------------------------------
# Calcular importancia de variables (VIP)
...
# --- 21. ANÁLISIS SHAP (CS) ---------------------------------------------------
# Extraer modelo XGBoost interno
...
# --- 22. FILTRADO PARA CANINOS INFERIORES (CI) --------------------------------
...
summary(ci_train)
summary(ci_test)
length(intersect(unique(ci_train$Ind), unique(ci_test$Ind)))  # Debe ser 0

# --- 24. PREPROCESAMIENTO (CI) ------------------------------------------------
# Receta para CI (mismas variables que CS)
...
# Métricas para CI
metricas <- metric_set(accuracy, sens, spec, bal_accuracy, roc_auc)
ctrl <- control_resamples(save_pred = TRUE)

# --- 25. MODELOS PARA CI (RESUMIDO) -------------------------------------------
# Modelo Logístico CI
...
log_res_CI <- fit_resamples(log_wf_CI, resamples = cv_ci, control = ctrl, metrics = metricas)
collect_metrics(log_res_CI)

# Modelo LDA CI
lda_spec_CI <- discrim_linear() %>% set_engine("MASS")
lda_wf_CI <- workflow() %>% add_recipe(ci_recipe) %>% add_model(lda_spec_CI)
lda_res_CI <- fit_resamples(lda_wf_CI, resamples = cv_ci, metrics = metricas, control = ctrl)
collect_metrics(lda_res_CI)

# Modelo Naive Bayes CI
nb_spec_CI <- naive_Bayes() %>% set_engine("klaR")
nb_wf_CI <- workflow() %>% add_recipe(ci_recipe) %>% add_model(nb_spec_CI)
nb_res_CI <- fit_resamples(nb_wf_CI, resamples = cv_ci, metrics = metricas, control = ctrl)
collect_metrics(nb_res_CI)

# Modelo Random Forest CI
rf_spec_CI <- rand_forest(trees = 1000, mtry = tune(), min_n = tune()) %>%
  set_engine("ranger") %>% set_mode("classification")
rf_wf_CI <- workflow() %>% add_recipe(ci_recipe) %>% add_model(rf_spec_CI)
rf_grid_CI <- grid_regular(mtry(range = c(1, 4)), min_n(range = c(2, 20)), levels = 6)
set.seed(123)
rf_res_CI <- tune_grid(rf_wf_CI, resamples = cv_ci, grid = rf_grid_CI, metrics = metricas, control = ctrl)
best_rf_CI <- select_best(rf_res_CI, metric = "bal_accuracy")

# Modelo Elastic Net CI
enet_spec_CI <- logistic_reg(penalty = tune(), mixture = tune()) %>%
  set_engine("glmnet") %>% set_mode("classification")
enet_wf_CI <- workflow() %>% add_recipe(ci_recipe) %>% add_model(enet_spec_CI)
enet_grid_CI <- grid_regular(penalty(range = c(-4, 0)), mixture(range = c(0, 1)), levels = 10)
set.seed(123)
enet_res_CI <- tune_grid(enet_wf_CI, resamples = cv_ci, grid = enet_grid_CI, control = ctrl, metrics = metricas)
best_enet_CI <- select_best(enet_res_CI, metric = "bal_accuracy")

# Modelo SVM Lineal CI
svm_lin_CI <- svm_linear(cost = tune()) %>% set_engine("kernlab") %>% set_mode("classification")
svm_lin_wf_CI <- workflow() %>% add_recipe(ci_recipe) %>% add_model(svm_lin_CI)
svm_lin_grid_CI <- grid_regular(cost(range = c(-3, 2)), levels = 10)
set.seed(123)
svm_lin_res_CI <- tune_grid(svm_lin_wf_CI, resamples = cv_ci, grid = svm_lin_grid_CI, metrics = metricas, control = ctrl)
best_svm_lin_CI <- select_best(svm_lin_res_CI, metric = "bal_accuracy")

# Modelo SVM Radial CI (MEJOR MODELO CI)
svm_rad_CI <- svm_rbf(cost = tune(), rbf_sigma = tune()) %>%
  set_engine("kernlab") %>% set_mode("classification")
svm_rad_wf_CI <- workflow() %>% add_recipe(ci_recipe) %>% add_model(svm_rad_CI)
svm_rad_grid_CI <- grid_regular(cost(range = c(-2, 2)), rbf_sigma(range = c(-3, -1)), levels = 6)
set.seed(123)
svm_rad_res_CI <- tune_grid(svm_rad_wf_CI, resamples = cv_ci, grid = svm_rad_grid_CI, metrics = metricas, control = ctrl)
best_svm_rad_CI <- select_best(svm_rad_res_CI, metric = "bal_accuracy")

# Modelo XGBoost CI
xgb_spec_CI <- boost_tree(trees = 800, learn_rate = tune(), mtry = tune(),
                          tree_depth = tune(), min_n = tune(), loss_reduction = tune()) %>%
  set_engine("xgboost") %>% set_mode("classification")
xgb_wf_CI <- workflow() %>% add_recipe(ci_recipe) %>% add_model(xgb_spec_CI)
xgb_grid_CI <- grid_space_filling(learn_rate(range = c(-3, -1)), mtry(range = c(1, 4)),
                                  tree_depth(range = c(2L, 6L)), min_n(range = c(2L, 15L)),
                                  loss_reduction(range = c(-5, -1)), size = 25)
set.seed(123)
xgb_res_CI <- tune_grid(xgb_wf_CI, resamples = cv_ci, grid = xgb_grid_CI, metrics = metricas, control = ctrl)
best_xgb_CI <- select_best(xgb_res_CI, metric = "bal_accuracy")

# --- 26. COMPARACIÓN DE MODELOS (CI) ------------------------------------------
...
# Bootstrap para intervalos de confianza (CI)
set.seed(123)
boot_res_CI <- boot(
  data = svm_pred_CI,
  statistic = boot_metricas,
...
# --- 28. IMPORTANCIA POR PERMUTACIÓN (CI) -------------------------------------
...
# --- 30. GUARDAR MODELOS Y DATOS FINALES --------------------------------------
# Guardar modelos finales
saveRDS(xgb_fit,        "xgb_fit_CS.rds")
saveRDS(svm_rad_fit_CI, "svm_rad_fit_CI.rds")

# Guardar conjuntos de prueba (nombres consistentes)
